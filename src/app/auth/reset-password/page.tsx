...
function ResetPasswordContent() {
  ...
  useEffect(() => {
    ...
    // Came from callback page with verified session
    if (verified === '1') {
      setReady(true)
      return
    }
    ...
    // Direct access — check for existing session
    const supabase = createClient()
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        setReady(true)
      } else {
        // No session — redirect to callback which handles everything
        // Check if there's a hash fragment
        if (window.location.hash.includes('access_token')) {
          // Parse it here directly
    ...
    return (
      <div className="text-center">
        ...
        <p className="text-dark-400 text-sm font-body mb-5">
          Ce lien a expiré ou a déjà été utilisé.<br/>
  ...
  if (!ready) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-dark-400 text-sm font-body">Vérification en cours...</p>
      </div>
    )
  }
