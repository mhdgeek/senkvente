...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // Method 1: token_hash in URL query params (Supabase v2 default)
    ...
    // Method 2: Listen for PASSWORD_RECOVERY event from hash fragment
    ...
    // Method 3: Check existing session (user already authenticated via link)
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) setReady(true)
    })

    // Timeout — if nothing works after 5s, show error
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setError("Le lien a expiré ou est invalide. Faites une nouvelle demande.")
      setLoading(false)
      return
    }
  ...
  if (invalid) {
    return (
      <div className="text-center">
        ...
        <p className="text-dark-400 text-sm font-body mb-5">
  ...
  if (!ready) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-dark-400 text-sm font-body">Vérification du lien en cours...</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-dark-300 mb-1.5 font-body">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 py-3.5 bg-brand-600 hover:bg-brand-500 disabled:opacity-60 text-white font-semibold rounded-xl transition-all text-sm font-body mt-2"
        >
          {loading
