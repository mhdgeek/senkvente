...
import {
  PAYMENT_METHODS, INTERVENTION_TYPES, SUBSCRIPTION_SERVICES,
  SUBSCRIPTION_PERIODS
} from '@/types'
import {
  ArrowLeft, Loader2, ShoppingBag, Wrench, RefreshCw, CheckCircle2,
  UserPlus, X, TrendingUp, TrendingDown, Truck
} from 'lucide-react'
...
const TYPE_CONFIG = [
  {
    ...
    label: 'Vente',
    desc: "Vente d'un produit ou service",
    icon: ShoppingBag,
    color: 'text-blue-600',
    bg: 'bg-blue-50',
...
export default function NewSalePage() {
  ...
  const showMargin   = salePrice > 0 && (costPrice > 0 || deliveryCost > 0)

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.client_id) { setError("Veuillez sélectionner un client."); return }
    if (!form.amount || salePrice <= 0) { setError("Le prix de vente est invalide."); return }
    ...
    if (form.type === 'intervention') payload.intervention_type = form.intervention_type
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-up">
      <div className="mb-6">
        ...
        <p className="text-dark-400 text-sm mt-1 font-body">Enregistrez une vente, intervention ou abonnement</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {TYPE_CONFIG.map(tc => (
              <button
                key={tc.value}
                type="button"
                onClick={() => handleTypeSelect(tc.value)}
                className={cn(
                  'p-3 rounded-xl border-2 text-left transition-all duration-150',
                  form.type === tc.value
                    ? `${tc.activeBorder} ${tc.activeBg}`
                    : 'border-dark-200 hover:border-dark-300 bg-white'
                )}
              >
                <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center mb-2', tc.bg)}>
                  <tc.icon className={cn('w-4 h-4', tc.color)} />
                </div>
                <p className={cn('text-xs font-semibold font-body', form.type === tc.value ? tc.color : 'text-dark-700')}>
                  {tc.label}
                </p>
                <p className="text-xs text-dark-400 mt-0.5 font-body leading-tight">{tc.desc}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="card p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              {selectedClientName && (
                <p className="text-xs text-dark-400 mt-1.5 font-body flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                  {selectedClientName} sélectionné
                </p>
              )}
            </div>
            <div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder={
                form.type === 'intervention' ? "Ex: Installation caméra de surveillance, 4 caméras IP..." :
                form.type === 'abonnement' ? "Ex: Abonnement IPTV 200 chaînes, renouvellement mensuel..." :
                "Ex: Vente ordinateur portable HP Core i5..."
              }
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-dark-400 text-xs font-body">FCFA</span>
              </div>
            </div>
          ...
          {showMargin && (
            <div className={cn(
              'flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl border',
              ...
              <div>
                ...
                <p className={cn('text-xl font-bold font-display', margin >= 0 ? 'text-emerald-700' : 'text-red-700')}>
                  {formatCFA(margin)}
                </p>
              </div>
              <div>
                <p className="text-xs text-dark-500 font-body mb-0.5">Taux de marge</p>
                <p className={cn('text-xl font-bold font-display', marginPct >= 0 ? 'text-emerald-700' : 'text-red-700')}>
                  {marginPct}%
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      'py-2 px-3 text-xs font-medium rounded-lg border-2 transition-all font-body',
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={loading} className="btn-primary flex-1 justify-center">
            {loading
              ...
              : <><selectedType.icon className="w-4 h-4" /> Enregistrer la transaction</>
            }
          </button>
        </div>
      </form>

      {/* Inline new client modal — uses min-height wrapper to avoid fixed positioning */}
      {showNewClient && (
        <div
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-fade-up">
            ...
            <form onSubmit={handleCreateClient} className="p-5 space-y-4">
              <p className="text-xs text-dark-400 font-body">
                Ce client sera créé et automatiquement sélectionné pour cette transaction.
              </p>
              <div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {clientLoading
                    ...
                    : <><UserPlus className="w-4 h-4" /> Créer &amp; sélectionner</>
                  }
                </button>
              </div>
            </form>
